...
function handleGetConfig() {
  ...
  // Sertakan spreadsheet ID aktif supaya admin bisa lihat
  const currentId = PropertiesService.getScriptProperties().getProperty('SPREADSHEET_ID') || ss.getId();
  return ok({ config, available_years, userCount: uSh ? Math.max(0, uSh.getLastRow()-1) : 0, spreadsheet_id: currentId });
}
...
function handleGenerateNomor(b) {
  const { perihal, alamat, tanggal, tahun } = b;
  ...
  try {
    ...
    if (lastRow > 1) {
      ...
      let mx = 0;
      col.forEach(r => { const n = parseInt(r[0]); if (!isNaN(n) && n > mx) mx = n; });
      nextNo = mx + 1;
    }
    const tglFmt = formatTanggal(tanggal);
    const ts = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm:ss');
    // Urutan kolom: No_Surat | Alamat_Penerima | Tanggal_Surat | Perihal | Timestamp_Input | User_Email | User_Nama
    sh.appendRow([nextNo, alamat || '', tglFmt, perihal, ts, b.user_email || '', b.user_nama || '']);
    try { sh.autoResizeColumns(1, COL_HEADERS.length); } catch(_) {}
    return ok({ nomor_surat: nextNo, no_urut: nextNo, tanggal: tglFmt, timestamp: ts });
